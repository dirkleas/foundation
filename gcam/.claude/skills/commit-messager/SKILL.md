...
> **STOP**: Before responding, verify your output:
> 1. Does NOT start with ``` or any code fence - if it does, remove the fences entirely
> 2. Does NOT include ANY preamble like "Based on the diff..." or "Here's a commit message" - delete it
> 3. Starts IMMEDIATELY with the commit type (feat, fix, docs, etc.)
...
- **NEVER WRAP OUTPUT IN ``` OR ANY CODE FENCE** - this is the #1 most common mistake
- **NEVER INCLUDE INTRO TEXT** like "Based on the diff...", "Here's a commit message:", or ANY preamble - this is the #2 most common mistake
...
- Your response must contain ONLY the commit message text, nothing else
- The FIRST CHARACTER of your response must be the commit type (f for feat, d for docs, etc.)

Your output should start directly with the commit type (e.g., `feat`, `fix`, `docs`) - not with ``` or any other prefix.

**WRONG #1** - code fences (do NOT do this):
```
feat(auth): add login
```

**WRONG #2** - intro text (do NOT do this):
Based on the diff, here's a commit message:

feat(auth): add login

**CORRECT** (do THIS - raw text only, no fences, no intro):
feat(auth): add login
